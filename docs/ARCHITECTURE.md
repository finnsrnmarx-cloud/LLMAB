...
1. Ship an on-device multimodal macOS app (`ω`) that runs local LLMs — Gemma 4 first.
...
public protocol LLMRuntime: Sendable {
    var id: String { get }                             // "ollama" | "mlx" | "llamacpp"
...
`ChatRequest` carries the full message history, tool schema, image/audio/video parts, and sampling config. `ChatChunk` is either a token delta, a tool-call, or a terminal reason. The adapter layer turns this into Ollama's `/api/chat`, MLX's `mlx_lm.generate`, or llama-server's OpenAI-compatible endpoint.

## Capability gating

`ModelRegistry` holds the canonical Gemma 4 capability table and a heuristic pattern matcher (`gemma-4:e2b` / `gemma-4:e4b` / `gemma-4:26b` / `gemma-4:31b`). The UI asks `ModelRegistry.capabilities(for: selectedModel)` before enabling any tab sub-mode.
